 ...
 */
#define SM_PAUSE            0x2000  // note: Only available with patch. This only quickly pauses the VS's internal buffer, when canceling quickly. It won't unpause.
 ...
 * \see SM_LINE1
 */
// configure Line1 as single ended, otherwise as differential 10x gain for microphones (when commented out)!!
...
class SFEMP3Shield {
  public:
	...
	uint8_t VSLoadUserCode(char*);  //made public to use in user code to upload binary formatted plugins - MJK 2/2017
	static void Mp3WriteRegister(uint8_t, uint8_t, uint8_t);  //made public to use in user code to directly modify or read registers on the VS1053b - MJK 2/2017
    static void Mp3WriteRegister(uint8_t, uint16_t);  //made public to use in user code to directly modify or read registers on the VS1053b - MJK 2/2017
    static uint16_t Mp3ReadRegister (uint8_t);  //made public to use in user code to directly modify or read registers on the VS1053b - MJK 2/2017
    static uint16_t Mp3ReadWRAM(uint16_t);  //made public to use in user code to directly modify or read registers on the VS1053b - MJK 2/2017
    static void Mp3WriteWRAM(uint16_t, uint16_t);  //made public to use in user code to directly modify or read registers on the VS1053b - MJK 2/2017
